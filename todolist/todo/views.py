...
def edit_todo(request, pk):
    ...
    if request.method == 'POST':
        ...
        if status:
            ...
            status = 'in progress'
            messages.warning(request, 'Todo item marked as in progress!')
        todo_item.taskname = taskname
        todo_item.date = date
        todo_item.status = status
        todo_item.save()
...
#         task.delete()
#     else:
#         print(f"Task with pk {pk} does not exist")
#     return redirect('create_and_list_todo')
...
def finding(request, number):
    ...
    return render(request, "task_list.html", {"tasks": all_todos})
